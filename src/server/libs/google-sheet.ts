...
export class GoogleSheetRowClient<Headers extends Record<string, HeaderType>> {
  ...
  async prepare() {
    ...
    this.sheet = this.doc.sheetsById[this.sheetId] as GoogleSpreadsheetWorksheet;
  ...
  private processRow(row: GoogleSpreadsheetRow<Record<string, any>>) {
    ...
    for (const header of Object.keys(this.options.headers)) {
      const cellValue = this.processCell(row, header, this.options.headers[header] as HeaderType);
      if (cellValue.isSkip) {
        isSkip = true;
        break;
      }
